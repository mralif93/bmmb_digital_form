...
class RafFormSubmissionController extends Controller
{
    /**
    ...
    public function index(Request $request, RemittanceApplicationForm $form): View|JsonResponse
    {
        ...
        return view('admin.raf.submissions.index', compact('submissions', 'form'));
    }
    ...
    public function create(RemittanceApplicationForm $form): View
    {
        ...
        return view('admin.raf.submissions.create', compact('form', 'users'));
    }
    ...
    public function store(Request $request, RemittanceApplicationForm $form): RedirectResponse
    {
        ...
        $submissionNumber = 'RAF-SUB-' . date('Y') . '-' . str_pad(
            RafFormSubmission::whereYear('created_at', date('Y'))->count() + 1,
            6,
            '0',
            STR_PAD_LEFT
        );
        ...
        $submission = RafFormSubmission::create($validated);

        return redirect()->route('admin.raf.forms.submissions.show', [$form, $submission])
     ...
     */
    public function show(RemittanceApplicationForm $form, RafFormSubmission $submission): View
    {
        $submission->load(['user', 'reviewedBy', 'form']);
        
        return view('admin.raf.submissions.show', compact('form', 'submission'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(RemittanceApplicationForm $form, RafFormSubmission $submission): View
    {
        $users = User::where('status', 'active')->get();
        $form->load('formFields');

        return view('admin.raf.submissions.edit', compact('form', 'submission', 'users'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, RemittanceApplicationForm $form, RafFormSubmission $submission): RedirectResponse
    {
        $validated = $request->validate([
            'user_id' => 'required|exists:users,id',
     ...
     */
    public function destroy(RemittanceApplicationForm $form, RafFormSubmission $submission): RedirectResponse
    {
        $submission->delete();

        return redirect()->route('admin.raf.forms.submissions.index', $form)
     ...
     */
    public function updateStatus(Request $request, RemittanceApplicationForm $form, RafFormSubmission $submission): JsonResponse
    {
        $validated = $request->validate([
            'status' => 'required|in:submitted,under_review,approved,rejected,completed',
            'review_notes' => 'nullable|string',
     ...
     */
    public function assign(Request $request, RemittanceApplicationForm $form, RafFormSubmission $submission): JsonResponse
    {
        $validated = $request->validate([
     ...
     */
    public function export(Request $request, RemittanceApplicationForm $form)
    {
        ...
        $filename = 'raf_submissions_' . $form->id . '_' . date('Y-m-d_H-i-s') . '.csv';
        ...
        $callback = function() use ($submissions) {
            ...
            foreach ($submissions as $submission) {
                fputcsv($file, [
                    ...
                    $submission->status,
                    $submission->submission_data['applicant_name'] ?? '',
                    $submission->submission_data['applicant_email'] ?? '',
     ...
     */
    public function statistics(RemittanceApplicationForm $form): JsonResponse
    {
        $stats = [
     ...
     */
    public function bulkUpdate(Request $request, RemittanceApplicationForm $form): JsonResponse
    {
        $validated = $request->validate([
            'submission_ids' => 'required|array',
            'submission_ids.*' => 'exists:raf_form_submissions,id',
            'action' => 'required|in:approve,reject,assign,delete',
            'reviewed_by' => 'required_if:action,assign|exists:users,id',
            'review_notes' => 'nullable|string',
        ]);

        $submissions = RafFormSubmission::whereIn('id', $validated['submission_ids'])
