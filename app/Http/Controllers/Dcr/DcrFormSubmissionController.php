...
class DcrFormSubmissionController extends Controller
{
    /**
    ...
    public function index(Request $request, DataCorrectionRequestForm $form): View|JsonResponse
    {
        ...
        return view('admin.dcr.submissions.index', compact('submissions', 'form'));
    }
    ...
    public function create(DataCorrectionRequestForm $form): View
    {
        ...
        return view('admin.dcr.submissions.create', compact('form', 'users'));
    }
    ...
    public function store(Request $request, DataCorrectionRequestForm $form): RedirectResponse
    {
        ...
        $submissionNumber = 'DCR-SUB-' . date('Y') . '-' . str_pad(
            DcrFormSubmission::whereYear('created_at', date('Y'))->count() + 1,
            6,
            '0',
            STR_PAD_LEFT
        );
        ...
        $submission = DcrFormSubmission::create($validated);

        return redirect()->route('admin.dcr.forms.submissions.show', [$form, $submission])
     ...
     */
    public function show(DataCorrectionRequestForm $form, DcrFormSubmission $submission): View
    {
        $submission->load(['user', 'reviewedBy', 'form', 'correctionActions', 'verificationRecords']);
        
        return view('admin.dcr.submissions.show', compact('form', 'submission'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(DataCorrectionRequestForm $form, DcrFormSubmission $submission): View
    {
        $users = User::where('status', 'active')->get();
        $form->load('formFields');

        return view('admin.dcr.submissions.edit', compact('form', 'submission', 'users'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, DataCorrectionRequestForm $form, DcrFormSubmission $submission): RedirectResponse
    {
        $validated = $request->validate([
            'user_id' => 'required|exists:users,id',
     ...
     */
    public function destroy(DataCorrectionRequestForm $form, DcrFormSubmission $submission): RedirectResponse
    {
        $submission->delete();

        return redirect()->route('admin.dcr.forms.submissions.index', $form)
     ...
     */
    public function updateStatus(Request $request, DataCorrectionRequestForm $form, DcrFormSubmission $submission): JsonResponse
    {
        $validated = $request->validate([
            'status' => 'required|in:submitted,under_review,approved,rejected,completed',
            'review_notes' => 'nullable|string',
     ...
     */
    public function assign(Request $request, DataCorrectionRequestForm $form, DcrFormSubmission $submission): JsonResponse
    {
        $validated = $request->validate([
     ...
     */
    public function export(Request $request, DataCorrectionRequestForm $form)
    {
        ...
        $filename = 'dcr_submissions_' . $form->id . '_' . date('Y-m-d_H-i-s') . '.csv';
     ...
     */
    public function statistics(DataCorrectionRequestForm $form): JsonResponse
    {
        $stats = [
     ...
     */
    public function bulkUpdate(Request $request, DataCorrectionRequestForm $form): JsonResponse
    {
        $validated = $request->validate([
            'submission_ids' => 'required|array',
            'submission_ids.*' => 'exists:dcr_form_submissions,id',
            'action' => 'required|in:approve,reject,assign,delete',
            'reviewed_by' => 'required_if:action,assign|exists:users,id',
            'review_notes' => 'nullable|string',
        ]);

        $submissions = DcrFormSubmission::whereIn('id', $validated['submission_ids'])
