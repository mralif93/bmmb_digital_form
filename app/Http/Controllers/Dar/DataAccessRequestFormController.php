...
class DataAccessRequestFormController extends Controller
{
    /**
    ...
    {
        $query = DataAccessRequestForm::with(['user', 'processedBy'])
        ...
        if ($request->expectsJson()) {
            return response()->json([
                'forms' => $forms,
                'filters' => $request->only(['status', 'request_type', 'priority', 'date_from', 'date_to', 'search'])
            ]);
        }

        return view('admin.dar.forms.index', compact('forms'));
    }
    ...
    public function create(): View
    {
        ...
        $requestTypes = DataAccessRequestForm::getRequestTypes();
        $priorities = ['low', 'medium', 'high', 'urgent'];

        return view('admin.dar.forms.create', compact('users', 'requestTypes', 'priorities'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'requested_data_categories' => 'required|array',
            'requested_data_categories.*' => 'string|max:100',
            'requested_data_timeframe' => 'required|string|max:100',
            'request_purpose' => 'required|string|max:500',
            'request_justification' => 'required|string',
            'data_usage_description' => 'required|string',
        ...
        $form = DataAccessRequestForm::create($validated);

        return redirect()->route('admin.dar.forms.show', $form)
            ->with('success', 'Data Access Request Form created successfully.');
    }

    /**
     * Display the specified resource.
     */
    public function show(DataAccessRequestForm $form): View
    {
        $form->load(['user', 'processedBy', 'formFields', 'submissions', 'responseData']);
        
        return view('admin.dar.forms.show', compact('form'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(DataAccessRequestForm $form): View
    {
        $users = User::where('status', 'active')->get();
        $requestTypes = DataAccessRequestForm::getRequestTypes();
        $priorities = ['low', 'medium', 'high', 'urgent'];

        return view('admin.dar.forms.edit', compact('form', 'users', 'requestTypes', 'priorities'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, DataAccessRequestForm $form): RedirectResponse
    {
        $validated = $request->validate([
            'user_id' => 'required|exists:users,id',
        ...
        return redirect()->route('admin.dar.forms.show', $form)
            ->with('success', 'Data Access Request Form updated successfully.');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(DataAccessRequestForm $form): RedirectResponse
    {
        $form->delete();

        return redirect()->route('admin.dar.forms.index')
            ->with('success', 'Data Access Request Form deleted successfully.');
    }

    /**
     * Update the status of the specified resource.
     */
    public function updateStatus(Request $request, DataAccessRequestForm $form): JsonResponse
    {
        $validated = $request->validate([
            'status' => 'required|in:draft,submitted,under_review,approved,rejected,completed',
     ...
     */
    public function assign(Request $request, DataAccessRequestForm $form): JsonResponse
    {
        $validated = $request->validate([
    ...
    public function export(Request $request)
    {
        ...
        $filename = 'dar_forms_' . date('Y-m-d_H-i-s') . '.csv';
