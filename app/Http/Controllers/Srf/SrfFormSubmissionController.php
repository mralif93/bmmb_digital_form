...
class SrfFormSubmissionController extends Controller
{
    /**
    ...
    public function index(Request $request, ServiceRequestForm $form): View|JsonResponse
    {
        ...
        return view('admin.srf.submissions.index', compact('submissions', 'form'));
    }
    ...
    public function create(ServiceRequestForm $form): View
    {
        ...
        return view('admin.srf.submissions.create', compact('form', 'users'));
    }
    ...
    public function store(Request $request, ServiceRequestForm $form): RedirectResponse
    {
        ...
        $submissionNumber = 'SRF-SUB-' . date('Y') . '-' . str_pad(
            SrfFormSubmission::whereYear('created_at', date('Y'))->count() + 1,
            6,
            '0',
            STR_PAD_LEFT
        );
        ...
        $submission = SrfFormSubmission::create($validated);

        return redirect()->route('admin.srf.forms.submissions.show', [$form, $submission])
     ...
     */
    public function show(ServiceRequestForm $form, SrfFormSubmission $submission): View
    {
        $submission->load(['user', 'reviewedBy', 'form', 'serviceActions', 'serviceHistory']);
        
        return view('admin.srf.submissions.show', compact('form', 'submission'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(ServiceRequestForm $form, SrfFormSubmission $submission): View
    {
        $users = User::where('status', 'active')->get();
        $form->load('formFields');

        return view('admin.srf.submissions.edit', compact('form', 'submission', 'users'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, ServiceRequestForm $form, SrfFormSubmission $submission): RedirectResponse
    {
        $validated = $request->validate([
            'user_id' => 'required|exists:users,id',
     ...
     */
    public function destroy(ServiceRequestForm $form, SrfFormSubmission $submission): RedirectResponse
    {
        $submission->delete();

        return redirect()->route('admin.srf.forms.submissions.index', $form)
     ...
     */
    public function updateStatus(Request $request, ServiceRequestForm $form, SrfFormSubmission $submission): JsonResponse
    {
        $validated = $request->validate([
            'status' => 'required|in:submitted,under_review,approved,rejected,completed',
            'review_notes' => 'nullable|string',
     ...
     */
    public function assign(Request $request, ServiceRequestForm $form, SrfFormSubmission $submission): JsonResponse
    {
        $validated = $request->validate([
     ...
     */
    public function export(Request $request, ServiceRequestForm $form)
    {
        ...
        $filename = 'srf_submissions_' . $form->id . '_' . date('Y-m-d_H-i-s') . '.csv';
        ...
        $callback = function() use ($submissions) {
            ...
            foreach ($submissions as $submission) {
                fputcsv($file, [
                    ...
                    $submission->status,
                    $submission->submission_data['customer_name'] ?? '',
                    $submission->submission_data['customer_email'] ?? '',
     ...
     */
    public function statistics(ServiceRequestForm $form): JsonResponse
    {
        $stats = [
     ...
     */
    public function bulkUpdate(Request $request, ServiceRequestForm $form): JsonResponse
    {
        $validated = $request->validate([
            'submission_ids' => 'required|array',
            'submission_ids.*' => 'exists:srf_form_submissions,id',
            'action' => 'required|in:approve,reject,assign,delete',
            'reviewed_by' => 'required_if:action,assign|exists:users,id',
            'review_notes' => 'nullable|string',
        ]);

        $submissions = SrfFormSubmission::whereIn('id', $validated['submission_ids'])
