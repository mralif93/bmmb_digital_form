        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($forms as $form)
                    ...
                    @endphp
                    <div class="group relative bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 hover:border-primary-400 dark:hover:border-primary-500 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 flex flex-col h-full overflow-hidden">
                        <!-- Decorative Element -->
                        <div class="absolute top-0 right-0 w-24 h-24 bg-primary-100 dark:bg-primary-900/30 rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                        
                        <div class="relative flex flex-col h-full">
                            <!-- Icon -->
                            <div class="w-14 h-14 bg-primary-500 dark:bg-primary-600 rounded-xl flex items-center justify-center mb-4 transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg shadow-primary-500/30 dark:shadow-primary-900/30">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4 leading-relaxed flex-grow">
                                {{ $form->description ?? 'Complete this form to submit your request quickly and securely.' }}
                            </p>
                            
                            <!-- Button -->
                            <a href="{{ in_array($form->slug, ['raf', 'dar', 'dcr', 'srf']) ? route('public.forms.' . $form->slug) : route('public.forms.slug', $form->slug) }}" class="block w-full bg-primary-500 dark:bg-primary-600 hover:bg-primary-600 dark:hover:bg-primary-700 text-white text-center px-4 py-3 rounded-xl font-semibold transition-all duration-300 transform group-hover:scale-105 inline-flex items-center justify-center mt-auto shadow-md hover:shadow-lg text-sm">
                                <span>Start Form</span>
                                <i class='bx bx-right-arrow-alt ml-2 text-lg group-hover:translate-x-1 transition-transform'></i>
                            </a>
                        </div>
                    </div>
                @endforeach
