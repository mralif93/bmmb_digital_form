...
<div class="mb-4 flex items-center justify-between">
    ...
    <div class="flex items-center space-x-2">
        <form id="regenerate-all-form" action="{{ route('admin.qr-codes.regenerate-all') }}" method="POST" class="inline">
            @csrf
            <button type="button" onclick="confirmRegenerateAll()" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors">
                <i class='bx bx-refresh mr-1.5'></i>
                Regenerate All
            </button>
        </form>
        <a href="{{ route('admin.qr-codes.create') }}" class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold rounded-lg transition-colors">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">
                        <div class="text-xs text-gray-900 dark:text-white">
                            {{ $timezoneHelper->convert($qrCode->created_at)?->format('M d, Y') }}
                        </div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">
                            {{ $timezoneHelper->convert($qrCode->created_at)?->format('h:i A') }}
                        </div>
                    </td>
                    <td class="px-4 py-3 whitespace-nowrap">
                        @if($qrCode->last_regenerated_at)
                            <div class="text-xs text-gray-900 dark:text-white">
                                {{ $timezoneHelper->convert($qrCode->last_regenerated_at)?->format('M d, Y') }}
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ $timezoneHelper->convert($qrCode->last_regenerated_at)?->format('h:i A') }}
                            </div>
                        @else
                ...
                <tr>
                    <td colspan="7" class="px-4 py-8 text-center">
                        ...
                        <p class="text-xs text-gray-600 dark:text-gray-400 mb-4">Get started by creating your first QR code</p>
                        <a href="{{ route('admin.qr-codes.create') }}" class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold rounded-lg transition-colors">
                            <i class='bx bx-plus mr-1.5'></i>
...
@push('scripts')
<script>
function showQrCodePopup(imageUrl, qrCodeName) {
    Swal.fire({
        title: qrCodeName,
        html: `
            <div class="flex flex-col items-center">
                <img src="${imageUrl}" alt="QR Code" class="w-64 h-64 border border-gray-300 rounded-lg p-2 bg-white mb-4">
                <a href="${imageUrl}" download="${qrCodeName.replace(/[^a-z0-9]/gi, '_')}.png" 
