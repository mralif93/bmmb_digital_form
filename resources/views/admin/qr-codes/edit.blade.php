...
<div class="mb-4 flex items-center justify-end">
    <a href="{{ route('admin.qr-codes.show', $qrCode->id) }}" class="inline-flex items-center px-3 py-2 text-xs font-semibold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <form action="{{ route('admin.qr-codes.update', $qrCode->id) }}" method="POST" class="p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="branch" {{ old('type', $qrCode->type) == 'branch' ? 'selected' : '' }}>Branch</option>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="active" {{ old('status', $qrCode->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $qrCode->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="size" id="size" value="{{ old('size', $qrCode->size) }}" min="100" max="1000"
                    ...
                    <div>
                        ...
                        <select name="format" id="format"
                            ...
                            <option value="jpg" {{ old('format', $qrCode->format) == 'jpg' ? 'selected' : '' }}>JPG</option>
                        </select>
                        @error('format')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
            ...
            <div class="flex justify-end space-x-3 mt-6">
                <a href="{{ route('admin.qr-codes.show', $qrCode->id) }}" class="px-4 py-2 text-xs font-medium text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                    Cancel
                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initial preview - wait a bit for DOM to be fully ready
    setTimeout(function() {
        updatePreview();
    }, 100);
});
