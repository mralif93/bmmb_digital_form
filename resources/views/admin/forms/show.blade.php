...
@section('content')
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                    {{ $form->description ?? 'No description provided' }}</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Multiple
                    Submissions</label>
                <p class="text-xs text-gray-900 dark:text-white">
                    {{ $form->allow_multiple_submissions ? 'Allowed' : 'Not Allowed' }}</p>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Created At</label>
                <p class="text-xs text-gray-600 dark:text-gray-400">
                    {{ $timezoneHelper->convert($form->created_at)->format($dateFormat . ' ' . $timeFormat) }}</p>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Updated At</label>
                <p class="text-xs text-gray-600 dark:text-gray-400">
                    {{ $timezoneHelper->convert($form->updated_at)->format($dateFormat . ' ' . $timeFormat) }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-2 pt-4 border-t border-gray-200 dark:border-gray-700">
            <a href="{{ route('admin.form-builder.index', $form) }}"
                class="inline-flex items-center px-3 py-1.5 bg-purple-100 hover:bg-purple-200 text-purple-700 dark:bg-purple-900/30 dark:hover:bg-purple-900/50 dark:text-purple-400 rounded-lg text-xs transition-colors">
                <i class='bx bx-code-alt mr-1'></i>
                Form Builder
            </a>
            <a href="{{ route('admin.form-sections.index', $form) }}"
                class="inline-flex items-center px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 dark:text-indigo-400 rounded-lg text-xs transition-colors">
                <i class='bx bx-list-ul mr-1'></i>
                Sections
            </a>
            <a href="{{ route('admin.forms.edit', $form) }}"
                class="inline-flex items-center px-3 py-1.5 bg-orange-100 hover:bg-orange-200 text-orange-700 dark:bg-orange-900/30 dark:hover:bg-orange-900/50 dark:text-orange-400 rounded-lg text-xs transition-colors">
    ...
    @else
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
            ...
            <div class="flex items-center justify-center space-x-2">
                <a href="{{ route('admin.form-sections.index', $form) }}"
                    class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition-colors">
