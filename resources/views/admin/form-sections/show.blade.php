...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    <div class="space-y-4">
        ...
        <div>
            ...
            <p class="text-xs text-gray-600 dark:text-gray-400">{{ $section->section_description ?? 'No description' }}</p>
        </div>
        ...
        <div>
            ...
            <span class="px-2 py-1 text-xs font-medium rounded-lg {{ $section->is_active ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' }}">
        ...
        <div>
            ...
            <p class="text-xs text-gray-600 dark:text-gray-400">{{ $section->created_at->format('Y-m-d H:i:s') }}</p>
        </div>

        <div>
            <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Updated At</label>
            <p class="text-xs text-gray-600 dark:text-gray-400">{{ $section->updated_at->format('Y-m-d H:i:s') }}</p>
        </div>
    </div>
