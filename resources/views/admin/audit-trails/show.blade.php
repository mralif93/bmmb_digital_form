...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                ...
                <dd class="text-xs text-gray-900 dark:text-white text-right flex-1">{{ $auditTrail->description ?? 'N/A' }}</dd>
            </div>
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
            ...
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                ...
                <dd class="text-xs text-gray-900 dark:text-white text-right flex-1">
                    {{ $timezoneHelper->convert($auditTrail->created_at)?->format('Y-m-d H:i:s') }}
                </dd>
            </div>
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        ...
        <div class="space-y-3">
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                ...
                <dd class="text-xs text-gray-900 dark:text-white text-right flex-1">{{ $auditTrail->ip_address ?? 'N/A' }}</dd>
            </div>
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                <dt class="text-xs font-semibold text-gray-700 dark:text-gray-300 pr-4">User Agent</dt>
                <dd class="text-xs text-gray-900 dark:text-white text-right flex-1 break-all">{{ $auditTrail->user_agent ?? 'N/A' }}</dd>
            </div>
            <div class="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                <dt class="text-xs font-semibold text-gray-700 dark:text-gray-300 pr-4">URL</dt>
                <dd class="text-xs text-gray-900 dark:text-white text-right flex-1 break-all">{{ $auditTrail->url ?? 'N/A' }}</dd>
            </div>
            <div class="flex items-start justify-between pb-2">
                <dt class="text-xs font-semibold text-gray-700 dark:text-gray-300 pr-4">HTTP Method</dt>
                <dd class="text-xs text-gray-900 dark:text-white text-right flex-1">{{ $auditTrail->method ?? 'N/A' }}</dd>
            </div>
        </div>
    </div>
    ...
    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Field</th>
                        <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Old Value</th>
                        <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">New Value</th>
                    </tr>
                </thead>
                    ...
                    <tr class="{{ $hasChanged ? 'bg-yellow-50 dark:bg-yellow-900/10' : '' }}">
                        ...
                        <td class="px-3 py-2">
                            ...
                            @elseif(is_array($oldValue) || is_object($oldValue))
                                <pre class="text-xs bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-2 rounded text-gray-900 dark:text-white overflow-x-auto max-w-xs">{{ json_encode($oldValue, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                            @else
                                <span class="text-xs text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded inline-block">{{ is_bool($oldValue) ? ($oldValue ? 'true' : 'false') : (string)$oldValue }}</span>
                            @endif
                        </td>
                        <td class="px-3 py-2">
                            @if($isDeleted)
                            ...
                            @elseif(is_array($newValue) || is_object($newValue))
                                <pre class="text-xs bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-2 rounded text-gray-900 dark:text-white overflow-x-auto max-w-xs">{{ json_encode($newValue, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                            @else
                                <span class="text-xs text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded inline-block">{{ is_bool($newValue) ? ($newValue ? 'true' : 'false') : (string)$newValue }}</span>
                            @endif
                        </td>
                    </tr>
                    @endforeach
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <pre class="text-xs bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-3 rounded-lg overflow-x-auto text-gray-900 dark:text-white">{{ json_encode($auditTrail->old_values, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
            </div>
            @endif
            ...
            <div>
                ...
                <pre class="text-xs bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-3 rounded-lg overflow-x-auto text-gray-900 dark:text-white">{{ json_encode($auditTrail->new_values, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
            </div>
            @endif
        </div>
        @endif
    ...
    <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        ...
        <pre class="text-xs bg-gray-100 dark:bg-gray-700 p-3 rounded-lg overflow-x-auto text-gray-900 dark:text-white">{{ json_encode($auditTrail->request_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
    </div>
    @endif
</div>
@endsection
