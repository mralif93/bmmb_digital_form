                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            @if($submission->submitted_at)
                                <div>{{ $timezoneHelper->convert($submission->submitted_at)?->format($dateFormat) ?? '-' }}</div>
                                <div class="text-xs text-gray-400">{{ $timezoneHelper->convert($submission->submitted_at)?->format($timeFormat) ?? '' }}</div>
                            @else
                                <div>{{ $timezoneHelper->convert($submission->created_at)?->format($dateFormat) }}</div>
                                <div class="text-xs text-gray-400">{{ $timezoneHelper->convert($submission->created_at)?->format($timeFormat) }}</div>
                            @endif
                        </td>
