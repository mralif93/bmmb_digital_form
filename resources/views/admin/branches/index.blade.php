    ...
    <form method="GET" action="{{ route('admin.branches.index') }}" class="space-y-3">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select name="state" 
                    ...
                    @foreach(\App\Models\Branch::distinct()->orderBy('state')->pluck('state') as $state)
            ...
            <!-- Region Filter -->
            <div>
                <label for="region" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Region</label>
                <select name="region" 
                        id="region"
                        class="w-full px-3 py-2 text-xs border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    <option value="">All Regions</option>
                    @foreach(\App\Models\Branch::distinct()->orderBy('region')->pluck('region') as $region)
        ...
        <div class="flex items-center justify-end space-x-2">
            @if(request()->hasAny(['search', 'state', 'region']))
                <a href="{{ route('admin.branches.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-300 text-xs font-semibold rounded-lg transition-colors">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-right text-xs font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            <a href="{{ route('admin.branches.show', $branch->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 bg-blue-100 hover:bg-blue-200 text-blue-700 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-400 rounded-lg text-xs transition-colors">
                                View
                            </a>
                            <a href="{{ route('admin.branches.edit', $branch->id) }}" class="inline-flex items-center justify-center px-3 py-1.5 bg-orange-100 hover:bg-orange-200 text-orange-700 dark:bg-orange-900/30 dark:hover:bg-orange-900/50 dark:text-orange-400 rounded-lg text-xs transition-colors">
                                Edit
                            </a>
                            <button onclick="deleteBranch({{ $branch->id }})" class="inline-flex items-center justify-center px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 rounded-lg text-xs transition-colors">
                                Delete
                            </button>
                        </div>
                    </td>
