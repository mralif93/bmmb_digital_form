...
/**
 * Export DCR Form Structure to JSON
 * 
 * This script exports the current DCR form structure from the database
 * to exports/dcr_form_export.json for use in the FormManagementSeeder.
 * 
 * Usage:
...
$form = Form::where('slug', 'dcr')->with(['sections.fields' => function($q) { 
    $q->orderBy('sort_order'); 
}])->first();

if (!$form) {
    echo "Error: DCR form not found in database.\n";
    echo "Please seed the form first by uncommenting seedDcrForm() in FormManagementSeeder.\n";
    exit(1);
}
...
$jsonFile = __DIR__ . '/exports/dcr_form_export.json';
...
echo "✓ DCR form exported successfully!\n";
echo "  File: {$jsonFile}\n";
echo "  Form: {$form->name}\n";
